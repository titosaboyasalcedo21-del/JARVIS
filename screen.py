...
"""
        try:
            ...
            log.debug("Window listing failed: %s", e)
            return []
    else:
        ...
        try:
            ...
            log.debug("wmctrl window listing failed: %s", e)
            return []
...
async def describe_screen(anthropic_client) -> str:
    ...
    if screenshot_b64 and anthropic_client:
        try:
            ...
            log.debug("Vision analysis failed: %s", e)
    ...
    if anthropic_client:
        try:
            ...
            log.debug("Window context summarization failed: %s", e)
